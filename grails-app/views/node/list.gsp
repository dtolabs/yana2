			...
			<table width=100% valign=top>
				...
				<g:each in="${nodeInstanceList}" status="i" var="nodeInstance">
					<tr>
						...
						<td style="padding-left:5px;" width=175 valign=top><g:link controller="node" action="show" id="${nodeInstance.id}">${fieldValue(bean: nodeInstance, field: "name")}</g:link> [<g:link controller="nodeType" action="show" id="${nodeInstance.nodetype.id}">${nodeInstance.nodetype.name}</g:link>]
