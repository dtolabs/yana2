...
curl --silent --fail \
    ...
    ${YANA_URL}/api/nodeAttribute/list/XML?project=${PROJECT} \
...
curl --silent --fail \
    --request DELETE \
    "${YANA_URL}/api/nodeAttribute/none/{$nodeAttributeIDs}?project=${PROJECT}" \
...
http_code=$(curl --silent --fail -w "%{http_code}" --request DELETE \
    ${YANA_URL}/api/nodeType/none/${ID}?project=${PROJECT} \
    --cookie ${cookie} -o $response) 
case ${http_code} in
    20*|404|410)
